...
ediamMSPE<-function(x,verbose=FALSE){
 calib.params<<-c(
                   ...
                   Gamma_re.N =  round(x[3],4),
                   Gamma_ce.N =   round(x[4],4),
                   ...
                   alfa.N = round(x[15],4),
                   alfa.S = round(x[16],4),
                   Gamma_re.S =  round(x[17],4),
                   Gamma_ce.S =   round(x[18],4)
                   )
...
#save calibrated parameters
  calib.params<<-c(
                  ...
                  Gamma_re.N =  round(out$par[3],4),
                  Gamma_ce.N =   round(out$par[4],4),
                  ...
                  Gamma_re.S =  round(out$par[17],4),
                  Gamma_ce.S =   round(out$par[18],4)
                  )
...
#Compare graphically solution found vs historical record using the following routine
 #load calibration params
   dir.bootstrap<-paste0(dir.calib,"\\Bootstrap_Out_Exp\\")
...
SimulData<-apply(params,1,function(x) {
                                pars<-c(
                                           ...
                                           lrng.re.N = round(as.numeric(x["lrng.re.N"]),4), #Note: no experience accumulation effects considered in this experiment
                                           lrng.ce.N = round(as.numeric(x["lrng.ce.N"]),4), #Note: no experience accumulation effects considered in this experiment
                                           lrng.re.S = round(as.numeric(x["lrng.re.S"]),4), #Note: no experience accumulation effects considered in this experiment
                                           lrng.ce.S = round(as.numeric(x["lrng.ce.S"]),4), #Note: no experience accumulation effects considered in this experiment
...
pars<- c( epsilon.N =2.27,  #epsilon.N
                    epsilon.S = 2.91, #epsilon.S
                     #0.024, #Gamma_re
                     #0.006, #Gamma_ce
                     Gamma_re.N= 0.024, #Gamma_re.N
                     Gamma_ce.N =0.006, #Gamma_ce.N
                     Eta_re.N =0.484, #Eta_re.N
                    Eta_ce.N = 0.64, #Eta_ce.N
                     Eta_re.S = 0.052, #Eta_re.S
                     Eta_ce.S = 0.026, #, #Eta_ce.S
                     val.param.N =1.0,   #val.param.N
                     val.param.S = 1.0,   #val.param.S
                     lrng.re.N =0.000 , #lrng.re.N
                     lrng.ce.N = 0.000 ,#, #lrng.ce.N
                     lrng.re.S = 0.000 , #lrng.re.N
                     lrng.ce.S = 0.000 , #lrng.ce.S
                    alfa.N = 0.333, #alfa.N
                     alfa.S = 0.333, #alfa.S
                     Gamma_re.S = 0.024, #Gamma_re.S
                     Gamma_ce.S = 0.006 #Gamma_ce.S
                   )
