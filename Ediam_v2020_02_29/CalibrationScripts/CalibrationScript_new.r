#in server
# for paper https://science.sciencemag.org/content/368/6486/36

root<-"D:\\2. Papers\\9.  EDIAM\\Model\\"
model.version<-"Ediam_v2020_02_29"
...
#Load script for running the historical calibration
  source(paste(dir.calib,"ediam_HistoricCalib_original.r",sep=""))
#Load script for determining initial conditions
  source(paste(dir.calib,"ediam_InitialConditions_original.r",sep=""))
#Load script for determining empirical parameters
  source(paste(dir.calib,"empirical_parameters.r",sep=""))
#Load EDIAM Equations
  #source(paste(dir.model,"ediam_Equations_discrete_new.r",sep=""))
  source(paste(dir.model,"ediam_Equations_discrete_original.r",sep=""))
...
ediamMSPE<-function(x,verbose=FALSE){
 calib.params<<-c(
                   ...
                   alfa.N = round(x[11],4), #0.33
                   alfa.S = round(x[12],4) #0.25
                   )
...
#check constraints
   MSPE <- ifelse(calib.params['alfa.S']>calib.params['alfa.N'],1e3,MSPE)
   MSPE <- ifelse(SimulData[44,'s_re.S']>SimulData[44,'s_ce.S'],1e3,MSPE)
...
#Load script for running the historical calibration
  source(paste(dir.calib,"ediam_HistoricCalib_new.r",sep=""))
#Load script for determining initial conditions
  source(paste(dir.calib,"ediam_InitialConditions_new.r",sep=""))
#Load script for determining empirical parameters
  source(paste(dir.calib,"empirical_parameters.r",sep=""))
#Load EDIAM Equations
  source(paste(dir.model,"ediam_Equations_discrete_new.r",sep=""))
...
HistoricData.hp$Time<-HistoricData.hp$Time-1
...
#we have curious case; when I change the value of alfa to 0.33 in the south, then the result in the north changes, why?????
#-> reply: this happens because of the way we simulate productivity, solution add parameters for gamma too,

#we have an important empiral questionn, why are there mos scientist in the clean sector in the south?
#tkae both equations>
#Eta_re.S=0.0516, epsi_re.S=0.0625, Xtech_re.S=1107.622 , alfa.S=0.25, Profits_re0.S=10.71625
#Profits_re0.S<-Eta_re.S*epsi_re.S*((1-alfa.S)/alfa.S)*Xtech_re.S
#Eta_ce.S=0.0183, epsi_ce.S=0.03052740 , Xtech_ce.S=2083.940,alfa_1.S=0.5*alfa.S ,Profits_ce0.S=5.498979
#Profits_ce0.S<-Eta_ce.S*epsi_ce.S*((1-alfa_1.S)/alfa_1.S)*Xtech_ce.S

#solution is probably to try new differential equation.
#also why the optimizador is not respecting the bounds?
...
calib.params<-c(
           ...
           alfa.S = 0.33 #round(as.numeric(params[1,"alfa.S"]),4)
         )

#
out<-ediamCalib(calib.params,verbose=TRUE)
...
> subset(out,times==0)[,c("GDP.N","Y_re.N","Y_ce.N","Consumption.N","times")]
     GDP.N   Y_re.N   Y_ce.N Consumption.N times
1 16945.94 161.0532 3313.743      3862.016     0
> subset(HistData,Country=="Memo: OECD Total" & Time==1971)[,c("GDP","Y_re.h","Y_ce.h","Re.h","Time")] #do it better with the filter historic
         GDP   Y_re.h   Y_ce.h     Re.h Time
404 15833.08 162.5425 3169.915 3169.915 1971
>
> subset(out,times==0)[,c("GDP.S","Y_re.S","Y_ce.S","Consumption.S","times")]
     GDP.S   Y_re.S   Y_ce.S Consumption.S times
1 4739.915 559.0201 1582.256      2262.892     0
> subset(HistData,Country=="NonOECD" & Time==1971)[,c("GDP","Y_re.h","Y_ce.h","Time")] # do it better witht the filtered historic
         GDP   Y_re.h   Y_ce.h Time
6841 4185.41 562.6222 1579.831 1971
>
