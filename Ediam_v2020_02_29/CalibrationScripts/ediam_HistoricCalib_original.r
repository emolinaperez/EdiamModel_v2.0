...
#==========================================================================================================================================================================================================
 Parameters <- c(Run.ID = 1,
                ...
                #Technological paramters
                 ...
                 #Gamma_re.N = as.numeric(calib.params['Gamma_re.N']),
                 #Gamma_ce.N = as.numeric(calib.params['Gamma_ce.N']),
                 #Gamma_re.S = as.numeric(calib.params['Gamma_re.S']),
                 #Gamma_ce.S = as.numeric(calib.params['Gamma_ce.S']),
...
#Add additional parameters
  Parameters<-c(Parameters,policy.vector)

# Create table with empiral parameters
 empars<-apply(data.frame(t=c(1:(as.numeric(Parameters['EndTime'])+1))),1,function(x) {out<- ediamEmpiricalParameters(Parameters,x['t']);
                                            out$t<-x['t']-1;
                                          out } )

 empars<-do.call('rbind',empars)
...
#Use estimated parameters for setting initial conditions
 InitialConditions <-ediamInitialConditions(c(unlist(empars[1,]),Parameters[subset(names(Parameters),!(names(Parameters)%in%c('alfa.N','alfa.S','epsilon.N','epsilon.S')))]))
...
#create function for parameters
#elasticity of substitution
 epsilon.N.Data<<-approxfun( x = seq(0,Parameters['EndTime'],1), #times
                     y = empars$epsilon.N, # Actual data points
                     method = "linear",
                     rule = 2)

#
 epsilon.S.Data<<-approxfun( x = seq(0,Parameters['EndTime'],1), #times
                    y = empars$epsilon.S, # Actual data points
                    method = "linear",
                    rule = 2)

if (TRUE%in%is.na(empars$alfa_2.S)==FALSE)
{

#alfas
alfa.N.Data<<-approxfun( x = seq(0,Parameters['EndTime'],1), #times
                    y = empars$alfa.N, # Actual data points
                    method = "linear",
                    rule = 2)

#
alfa_2.N.Data<<-approxfun( x = seq(0,Parameters['EndTime'],1), #times
                    y = empars$alfa_2.N, # Actual data points
                    method = "linear",
                    rule = 2)
#
alfa.S.Data<<-approxfun( x = seq(0,Parameters['EndTime'],1), #times
                    y = empars$alfa.S, # Actual data points
                    method = "linear",
                    rule = 2)

#
alfa_2.S.Data<<-approxfun( x = seq(0,Parameters['EndTime'],1), #times
                    y = empars$alfa_2.S, # Actual data points
                    method = "linear",
                    rule = 2)

#final prices
Price.final.N.Data<<-approxfun( x = seq(0,Parameters['EndTime'],1), #times
                    y = empars$Price.final.N, # Actual data points
                    method = "linear",
                    rule = 2)

#
Price.final.S.Data<<-approxfun( x = seq(0,Parameters['EndTime'],1), #times
                    y = empars$Price.final.S, # Actual data points
                    method = "linear",
                    rule = 2)
