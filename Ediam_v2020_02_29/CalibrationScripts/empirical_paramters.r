...
#
ediamEmpiricalParameters(pars,1)

empars<-apply(data.frame(t=c(1:42)),1,function(x) {out<- ediamEmpiricalParameters(pars,x['t']);
                                            out$t<-x['t'];
                                          out } )

empars<-do.call('rbind',empars)

#tomorrow run a test of the model reading this params
