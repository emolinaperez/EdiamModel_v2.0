...
ediamInitialConditions<-function(alfa.N,alfa_1.N,alfa_2.N,phi.N,phi_1.N,epsilon.N,epsi.N,
                                 alfa.S,alfa_1.S,alfa_2.S,phi.S,phi_1.S,epsilon.S,epsi.S,
                                 L_0.N,L_0.S,
                                 Y_0.N,Y_0.S,
...
#In the advanced region
 ...
 A_ce0.N <- ( ( Y_re0.N * ( (alfa.N^(2*alfa.N*(1-epsilon.N)))*((Y_re0.N/(teta.N*Y_ce0.N))^(phi.N/(epsilon.N*(1-alfa.N)))) +
...
#
 A_ce0.S <- ( ( Y_re0.S * ( (alfa.S^(2*alfa.S*(1-epsilon.S)))*((Y_re0.S/(teta.S*Y_ce0.S))^(phi.S/(epsilon.S*(1-alfa.S)))) +
...
#Define vector of intital conditions
  InitialConditions <- c(A_re = max(A_re0.N,A_re0.S),
