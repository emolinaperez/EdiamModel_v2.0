...
#Set parameters
 ...
 dir.prim<-paste(root,"RDM Outputs\\ClimateExperimentCh4_Final\\",sep="")
...
#create scenario bins 
 ...
 prim.data[,DiscountRate.bin:=ifelse(rho<0.0046,'low.rho',ifelse(rho<0.009,'medium.rho','high.rho'))]
...
#Combine all bins into one single combination 
 ...
 prim.data[,bin.type:=paste(epsilon.bin,Relative.Eta.bin,Relative.Gamma.bin,Relative.Nu.bin,Climate.bin,DiscountRate.bin)]

#make format adjustment 
...
#subset to policies of interest
  prim.data<-subset(prim.data,prim.data$least.cost.policy%in%c("P1","P2","P3","P4","P5","P6","P7"))
 #prim.data<-subset(prim.data,prim.data$least.cost.policy%in%c("P1","P2","P4","P6","P7"))
 #prim.data<-subset(prim.data,prim.data$least.cost.policy%in%c("P1","P2","P7"))
 #prim.data<-subset(prim.data,prim.data$least.cost.policy%in%c("P1","P7"))
...
#objective<-"Stabilization.Cost.Zero" 
#regret.objective<-"Total.Policy.Cost.100"
#density.threshold<-0.5
 
#######################################################################
#regret table
regret.objective<-"Total.Policy.Utility"
prim.regret<-data.frame(prim.data[,c("Future.ID",regret.objective),with=FALSE])
futures.key<-data.frame(Future.ID=unique(prim.regret$Future.ID))
futures.regret<-apply(futures.key,1,function(x) { pivot<-subset(prim.regret,prim.regret$Future.ID==x);
                              subset(pivot[,c("Future.ID",regret.objective)],pivot[,regret.objective]==max(pivot[,regret.objective]))
                                        })
futures.regret<-do.call("rbind",futures.regret)
colnames(futures.regret)<-c("Future.ID","Best.Result")
futures.regret<-data.table(futures.regret)
#################################################################################
prim.data<-merge(prim.data,futures.regret,by="Future.ID")
prim.data[,Negative.Regret:=get(regret.objective)-Best.Result]
...
robust.mapping<-function(prim.data,objective,regret.objective,density.threshold) {
 ...
 #firts estimate number of futures per bin
 prim.data[,future.count:=1]
##calculate the total cost per policy per bin
 bins.policy.table<-as.data.frame(prim.data[, j=list(Total.Policy.Cost.100=sum(Total.Policy.Cost.100),
																...
																Objective.cases=sum(get(objective)),
																Total.futures=sum(future.count),
																Negative.Regret=sum(Negative.Regret)),
                                    by = list( bin.type,
									           least.cost.policy)])
#calculate density and coverage
 bins.policy.table$density.objective<-bins.policy.table$Objective.cases/bins.policy.table$Total.futures
 bins.policy.table$coverage.objective<-bins.policy.table$Objective.cases/total.succesful
 all.bins<-as.character(unique(bins.policy.table$bin.type))
#keep policies that meet the density threshold
 bins.policy.table<-subset(bins.policy.table,bins.policy.table$density.objective>density.threshold)

#map the corresponding option
 bins.key<-data.frame(bin.type=as.character(unique(bins.policy.table$bin.type)))
 
 if (nrow(bins.key)>0)
 {
 bins.objective<-apply(bins.key,1,function(x) { pivot<-subset(bins.policy.table,bins.policy.table$bin.type==x);
                              subset(pivot[,c("bin.type","least.cost.policy","density.objective","coverage.objective")],pivot[,regret.objective]==max(pivot[,regret.objective]))
                                        })
 bins.objective<-do.call("rbind",bins.objective)
 bins.no.objecive<-data.frame(bin.type=subset(all.bins,!(all.bins%in%bins.objective$bin.type)),
                              least.cost.policy='other',
							  density.objective=0.0,
							  coverage.objective=0.0)
#rbind both tables
 bins.objective<-rbind(bins.objective,bins.no.objecive)
 colnames(bins.objective)<-c("bin.type",paste("policy.",objective,sep=""),paste("density.",objective,sep=""),paste("coverage.",objective,sep=""))
 }else {
 bins.objective<-data.frame(bin.type=all.bins,
                              least.cost.policy='other',
							  density.objective=0.0,
							  coverage.objective=0.0)
 colnames(bins.objective)<-c("bin.type",paste("policy.",objective,sep=""),paste("density.",objective,sep=""),paste("coverage.",objective,sep=""))
...
#regret.objective<-"Total.Policy.Cost.100"
regret.objective<-"Negative.Regret"
...
#change policy names to characters: 
 robust.mapping[,paste("policy.","Temp.Threshold.2C",sep="")]<-as.character(robust.mapping[,paste("policy.","Temp.Threshold.2C",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.Zero",sep="")]<-as.character(robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.Zero",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.5Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.5Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.10Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.10Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.15Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.15Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.20Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.20Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.25Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.25Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.30Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.30Percent",sep="")])

 robust.mapping[,paste("policy.","Stabilization",sep="")]<-as.character(robust.mapping[,paste("policy.","Stabilization",sep="")])
 robust.mapping[,paste("policy.","Stabilization.Cost.Zero",sep="")]<-as.character(robust.mapping[,paste("policy.","Stabilization.Cost.Zero",sep="")])
 robust.mapping[,paste("policy.","Stabilization.Cost.5Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Stabilization.Cost.5Percent",sep="")])
 robust.mapping[,paste("policy.","Stabilization.Cost.10Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Stabilization.Cost.10Percent",sep="")])
 robust.mapping[,paste("policy.","Stabilization.Cost.15Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Stabilization.Cost.15Percent",sep="")])
 robust.mapping[,paste("policy.","Stabilization.Cost.20Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Stabilization.Cost.20Percent",sep="")])
 robust.mapping[,paste("policy.","Stabilization.Cost.25Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Stabilization.Cost.25Percent",sep="")])
 robust.mapping[,paste("policy.","Stabilization.Cost.30Percent",sep="")]<-as.character(robust.mapping[,paste("policy.","Stabilization.Cost.30Percent",sep="")])

 

 
#cross reference policy actions 
 robust.mapping[,paste("policy.","Temp.Threshold.2C",sep="")]<-ifelse(robust.mapping[,paste("policy.","Stabilization",sep="")]=='other',robust.mapping[,paste("policy.","Temp.Threshold.2C",sep="")],robust.mapping[,paste("policy.","Stabilization",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.Zero",sep="")]<-ifelse(robust.mapping[,paste("policy.","Stabilization.Cost.Zero",sep="")]=='other',robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.Zero",sep="")],robust.mapping[,paste("policy.","Stabilization.Cost.Zero",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.5Percent",sep="")]<-ifelse(robust.mapping[,paste("policy.","Stabilization.Cost.5Percent",sep="")]=='other',robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.5Percent",sep="")],robust.mapping[,paste("policy.","Stabilization.Cost.5Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.10Percent",sep="")]<-ifelse(robust.mapping[,paste("policy.","Stabilization.Cost.10Percent",sep="")]=='other',robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.10Percent",sep="")],robust.mapping[,paste("policy.","Stabilization.Cost.10Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.15Percent",sep="")]<-ifelse(robust.mapping[,paste("policy.","Stabilization.Cost.15Percent",sep="")]=='other',robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.15Percent",sep="")],robust.mapping[,paste("policy.","Stabilization.Cost.15Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.20Percent",sep="")]<-ifelse(robust.mapping[,paste("policy.","Stabilization.Cost.20Percent",sep="")]=='other',robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.20Percent",sep="")],robust.mapping[,paste("policy.","Stabilization.Cost.20Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.25Percent",sep="")]<-ifelse(robust.mapping[,paste("policy.","Stabilization.Cost.25Percent",sep="")]=='other',robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.25Percent",sep="")],robust.mapping[,paste("policy.","Stabilization.Cost.25Percent",sep="")])
 robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.30Percent",sep="")]<-ifelse(robust.mapping[,paste("policy.","Stabilization.Cost.30Percent",sep="")]=='other',robust.mapping[,paste("policy.","Temp.Threshold.2C.Cost.30Percent",sep="")],robust.mapping[,paste("policy.","Stabilization.Cost.30Percent",sep="")])

#add columns for decision tree
 length.link<-length(seq(-3,4,by=0.5))
 #link.table<-data.frame(Link="link",T=rep(seq(-3,4,by=0.5),2),Path=c(seq(0,length.link-1,by=1),seq(length.link*2-1,length.link,by=-1)),Way.of.the.walk=c(rep('Min',length.link),rep('Max',length.link)))
 link.table<-data.frame(Link="link",T=rep(seq(-3,4,by=0.5),2))
 
#merge all into robust.mapping
 dim(robust.mapping)
 dim(link.table) 
 robust.mapping<-merge(robust.mapping,link.table)
 dim(robust.mapping) 
