...
ModelEngine <- function(Time, State, Parameters) {
  ...
  #Economic structure
   ...
   #Welfare Calculations
     ...
     Consumption.S<-Y.S-epsi.re*(1+Tec.subsidy.S)*Agg.demand.re.tech.S-epsi.ce*Agg.demand.ce.tech.S 
     lambda.S<-lambda.S 
     ...
     Utility.Consumer.N<-(-1*((Cost.S.Damage*Consumption.N)^(1-sigma.utility))/(1-sigma.utility))/((1+rho)^Time)
     Utility.Consumer.S<-(-1*((Cost.S.Damage*Consumption.S)^(1-sigma.utility))/(1-sigma.utility))/((1+rho)^Time)
