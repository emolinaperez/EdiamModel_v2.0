...
#in pc
  dir.model<<-"C:\\Users\\L03054557\\Edmundo-ITESM\\Proyectos\\Disertation\\Model Disertation\\TechChange Model\\Ediam_v23_09_2017\\"
  dir.data<-"C:\\Users\\L03054557\\Edmundo-ITESM\\Proyectos\\Disertation\\Model Disertation\\TechChange Model\\Ediam_v23_09_2017\\CalibrationScripts\\CalibrationData\\"
#
...
#test mode
#  calib.params<<-c( alfa.N = round(0.33,2),
#                    alfa_1.N = round(0.5,2),
#                    epsilon.N = round(5,2),
#                    alfa.S = round(0.33,2),
#                    alfa_1.S = round(0.5,2),
#                    epsilon.S = round(5,2),
#                    Gamma_re = round(0.02,2),
#                    Gamma_ce= round(0.02,2),
#                    Eta_re.N = round(0.9,2),
#                    Eta_ce.N = round(0.9,2),
#                    Eta_re.S = round(0.5,2),
#                    Eta_ce.S = round(0.5,2)
#                    )


#
 calib.params<<-c( alfa.N = round(x[1],2),
                   alfa_1.N = round(x[2],2),
                   epsilon.N = round(x[3],2),
                   alfa.S = round(x[4],2),
                   alfa_1.S = round(x[5],2),
                   epsilon.S = round(x[6],2),
                   Gamma_re = round(x[7],2),
                   Gamma_ce= round(x[8],2),
                   Eta_re.N = round(x[9],2),
                   Eta_ce.N = round(x[10],2),
                   Eta_re.S = round(x[11],2),
                   Eta_ce.S = round(x[12],2)
                   )
...
#Test
  ediamMSE(c( round(0.33,2), # alfa.N
              round(0.5,2), # alfa_1.N
              round(5,2), # epsilon.N
              round(0.33,2), # alfa.S
              round(0.5,2), # alfa_1.S
              round(5,2), # epsilon.S
              round(0.02,2), # Gamma_re
              round(0.02,2), # Gamma_ce
              round(0.9,2), # Eta_re.N
              round(0.9,2), # Eta_ce.N
              round(0.5,2), # Eta_re.S
              round(0.5,2) # Eta_ce.S
             ), verbose = TRUE
             )
...
#Define range of values
 ...
 lhs.sample$Gamma_re<-qunif(lhs.sample$Gamma_re,0.01,0.2)
 lhs.sample$Gamma_ce<-qunif(lhs.sample$Gamma_ce,0.01,0.2)
 lhs.sample$Eta_re.N<-qunif(lhs.sample$Eta_re.N,0.2,0.95)
 lhs.sample$Eta_ce.N<-qunif(lhs.sample$Eta_ce.N,0.2,0.95)
 lhs.sample$Eta_re.S<-qunif(lhs.sample$Eta_re.S,0.2,0.95)
 lhs.sample$Eta_ce.S<-qunif(lhs.sample$Eta_re.S,0.2,0.95)
...
global.elements<-list("ediamMSE","ediamCalib","HistData","HistGDPData","HistPrice","HistPopData","ValVars","dir.model")
