EdiamEquations <- function(t, State, Parameters, type = "wExhaustableResource") {
  with(as.list(c(State, Parameters, cR.Data)), {
...
#Second we determine the equilibrium conditions for each sector
     ...
     #Y.N<-(pi.N*(Y_re.N)^((epsilon.N-1)/epsilon.N)+(1-pi.N)*(Y_ce.N)^((epsilon.N-1)/epsilon.N))^(epsilon.N/(epsilon.N-1))
...
#NEED TO THINK WELL HOW TO MODEL RESEARCH ALLOCATION
          #s_re.N<-exp(Profits_re.N)/(exp(Profits_ce.N)+exp(Profits_re.N)) #alt 1 -did not work
          #s_re.N<-exp(Profits_re.N/Profits_ce.N)/(exp(Profits_ce.N/Profits_re.N)+exp(Profits_re.N/Profits_ce.N)) #alt 2 -did not work-
...
#Emerging Region
	#Auxiliaries in Emerging Region
     ...
     epsi_ce.S<-epsi.S #this is the cost of production of dirty technologies
	   L.S<-L.S.Data(t)#(exp(labor.growth.S*t))*L_0.S #the population of the South is 4.6 that of the North,
  ...
  #Second we determine the equilibrium conditions for each sector
    ...
    #Total Production
      ...
      #Y.S<-(pi.S*(Y_re.S)^((epsilon.S-1)/epsilon.S)+(1-pi.S)*(Y_ce.S)^((epsilon.S-1)/epsilon.S))^(epsilon.S/(epsilon.S-1))
    #Allocation of Scientists
#    if ( exp(Profits_re.S)==Inf | exp(Profits_ce.S)==Inf )
#    {
#      s_re.S<-ifelse(Profits_re.S > Profits_ce.S,1,0)
#      s_ce.S<-1-s_re.S
#     } else {
         #s_re.S<-exp(Profits_re.S)/(exp(Profits_ce.S)+exp(Profits_re.S))
         #s_re.S<-exp(Profits_re.S/Profits_ce.S)/(exp(Profits_ce.S/Profits_re.S)+exp(Profits_re.S/Profits_ce.S))
...
#Emerging Region
	#Auxiliaries in Emerging Region
#	   L.S<-(exp(labor.growth.S*t))*L_0.S #the population of the South is 4.6 that of the North,
  #First we determine the equilibrium levels of relative input prices and relative labour
