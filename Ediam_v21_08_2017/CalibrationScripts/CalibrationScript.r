...
#incloud
#  dir.model<<-"C:\\Users\\Administrator\\Documents\\Edmundo\\Projectos\\TechChange-RDM\\TechChange Model\\Ediam_v21_08_2017\\"
#  dir.data<-"C:\\Users\\Administrator\\Documents\\Edmundo\\Projectos\\TechChange-RDM\\TechChange Model\\Ediam_v21_08_2017\\CalibrationScripts\\CalibrationData\\"
#in pc
  dir.model<<-"C:\\Users\\L03054557\\Edmundo-ITESM\\Proyectos\\Disertation\\Model Disertation\\TechChange Model\\Ediam_v21_08_2017\\"
  dir.data<-"C:\\Users\\L03054557\\Edmundo-ITESM\\Proyectos\\Disertation\\Model Disertation\\TechChange Model\\Ediam_v21_08_2017\\CalibrationScripts\\CalibrationData\\"
#
  HistData<<-read.csv(paste(dir.data,"historic_energy_both_regions _v2.csv",sep="")) #units in [Quadrillion BTU]
  HistPrice<<-read.csv(paste(dir.data,"historic_energy_prices.csv",sep="")) #

 #from http://data.worldbank.org/indicator/SP.POP.TOTL
  HistPopData<-read.csv(paste(dir.data,"PopDataWorldBank.csv",sep="")) # units in people
  HistPopData<-data.frame( OECD = as.numeric((HistPopData[HistPopData$Country.Name =="OECD members",paste("X",c(1960:2016),sep="")]))/1e6,
                            World = as.numeric((HistPopData[HistPopData$Country.Name =="World",paste("X",c(1960:2016),sep="")]))/1e6 ,
                            Year = c(1960:2016) )
  HistPopData$NonOECD <- HistPopData$World - HistPopData$OECD
  HistPopData<<-subset(HistPopData,HistPopData$Year>=1980 & HistPopData$Year<=2012)

#GDP from https://data.worldbank.org/indicator/NY.GDP.MKTP.CD
  HistGDPData<-read.csv(paste(dir.data,"API_NY_GDP_DS2_WORLDBANK.csv",sep=""),skip=4) # units current US$
  HistGDPData<-data.frame( OECD = as.numeric((HistGDPData[HistGDPData$Country.Name =="OECD members",paste("X",c(1960:2016),sep="")]))/1e12, #trillion USD
                           World = as.numeric((HistGDPData[HistGDPData$Country.Name =="World",paste("X",c(1960:2016),sep="")]))/1e12, #trillion USD
                           Year = c(1960:2016) )
  HistGDPData$NonOECD <- HistGDPData$World - HistGDPData$OECD
  HistGDPData<<-subset(HistGDPData,HistGDPData$Year>=1980 & HistGDPData$Year<=2012)


#Energy intensity from https://data.worldbank.org/topic/energy-and-mining
#  HistElCons<-read.csv(paste(dir.data,"API_5_DS2_en_csv_v2_EnergyIntensity.csv",sep=""))  # units [kWh per capita]
#  HistElCons<-subset(HistElCons,HistElCons$Indicator.Code=="EG.USE.ELEC.KH.PC")
#  HistElCons<-data.frame( OECD = as.numeric((HistElCons[HistElCons$Country.Name =="OECD members",paste("X",c(1960:2016),sep="")])),
#                            World = as.numeric((HistElCons[HistElCons$Country.Name =="World",paste("X",c(1960:2016),sep="")])),
#                            Year = c(1960:2016) )

# subset to common years
#  HistElCons<-subset(HistElCons,HistElCons$Year>=1980 & HistElCons$Year<=2012)
# change to kwh
#  HistElCons$OECD<-HistElCons$OECD*HistPopData$OECD*1e6 # kwh
#  HistElCons$World<-HistElCons$World*HistPopData$World*1e6 # kwh
#  HistElCons$NonOECD <- HistElCons$World - HistElCons$OECD

#conversion to BTU; 1BTU = 0.00029307107017 [kwh], 1 Quadrillion = 1x10^15
 #HistElCons$OECD<-(HistElCons$OECD/0.00029307107017)/1e15 # Quadrillion btu
 #HistElCons$World<-(HistElCons$World/0.00029307107017)/1e15 # Quadrillion btu
 #HistElCons$NonOECD <- (HistElCons$NonOECD/0.00029307107017)/1e15 # Quadrillion btu
 #HistElCons <<- HistElCons

#electricity generation
  # 1990 7,304,508 Gwh

#primary energy supply 1990 OECD: 4525.760 [million toe]  1 toe = 3.968*1e7 BTU  ;(4525.760*1e6*3.968*1e7)/1e15



  ValVars<<-c("Y.N","Y_ce.N","Y.S","Y_ce.S")
...
#test mode
#  calib.params<<-c( pi.N = round(0.5,2),
#                    alfa.N = round(0.33,2),
#                    alfa_1.N = round(0.5,2),
#                    epsilon.N = round(5,2),
#                    pi.S = round(0.5,2),
#                    alfa.S = round(0.33,2),
#                    alfa_1.S = round(0.5,2),
#                    epsilon.S = round(5,2),
#                    Gamma_re = round(1.0,2),
#                    Gamma_ce= round(1.0,2),
#                    Eta_re = round(0.02,2),
#                    Eta_ce = round(0.02,2),
#                    Nu_re = round(0.02,2),
#                    Nu_ce = round(0.02,2)
...
#
 calib.params<<-c( pi.N = round(x[1],2),
                   alfa.N = round(x[2],2),
                   alfa_1.N = round(x[3],2),
                   epsilon.N = round(x[4],2),
                   pi.S = round(x[5],2),
                   alfa.S = round(x[6],2),
                   alfa_1.S = round(x[7],2),
                   epsilon.S = round(x[8],2),
                   Gamma_re = round(x[9],2),
                   Gamma_ce= round(x[10],2),
                   Eta_re = round(x[11],2),
                   Eta_ce = round(x[12],2),
                   Nu_re = round(x[13],2),
                   Nu_ce = round(x[14],2)
...
#Final output
    Y_h.N<<-HistGDPData$OECD/HistGDPData$OECD#[1]
    Y_h.S<<-HistGDPData$NonOECD/HistGDPData$NonOECD #[1]
#Population
...
#Estimate mean square differences
   MSEp1<-((SimulData[,ValVars[1]]-Y_h.N)/Y_h.N)^2
   MSEp1.1<-((SimulData[,ValVars[2]]-Y_ce.Nh)/Y_ce.Nh)^2
   MSEp2<-((SimulData[,ValVars[3]]-Y_h.S)/Y_h.S)^2
   MSEp2.1<-((SimulData[,ValVars[4]]-Y_ce.Sh)/Y_ce.Sh)^2

#Check constraints
   MSE<- ifelse(mean(MSEp1)<=0.0025,
                  ifelse(mean(MSEp1.1)<=0.0025,
                          ifelse(mean(MSEp2)<=0.0025,
                                   ifelse(mean(MSEp2.1)<=0.0025,
                                                        mean(c(MSEp1,MSEp1.1,MSEp2,MSEp2.1)),1e3)
                                ,1e3)
                        ,1e3)
                ,1e3)
#
  ...
  colnames(out)<-paste("MSE",c(ValVars[1],ValVars[2],ValVars[3],ValVars[4],"all"),sep=".")
  #check constraints
   #when I ran the entire simulation could check for a tolerance difference, for exampple:
    #MSEp1<=0.001
    #MSEp1.1<=0.001
    #MSEp2<=0.001
    #MSEp2<=0.001
if (verbose == TRUE)
{ return(out)
} else {return(as.numeric(out$MSE.all))}
...
#lets try optmizing this:
  ediamMSE(c(  round(0.01159775 ,2),
                           round(0.89663578 ,2),
                            round(5.52116609 ,2),
                            round(0.04577620 ,2),
                            round(0.77277290 ,2),
...
#before this to LHC sampling to know
 library(lhs)
 set.seed(5000)
 #sample.size<-1000000
 sample.size<-1000
 lhs.sample<-data.frame(randomLHS(sample.size,14),Run.ID=1:sample.size)
 colnames(lhs.sample)<-c("pi.N","alfa.N","alfa_1.N","epsilon.N","pi.S","alfa.S","alfa_1.S","epsilon.S","Gamma_re","Gamma_ce","Eta_re","Eta_ce","Nu_re","Nu_ce","Run.ID")
#
 lhs.sample$pi.N<-qunif(lhs.sample$pi.N,0.1,0.9)
 lhs.sample$alfa.N<-qunif(lhs.sample$alfa.N,0.3,0.35)
 lhs.sample$alfa_1.N<-qunif(lhs.sample$alfa_1.N,0.1,0.9)
 lhs.sample$epsilon.N<-qunif(lhs.sample$epsilon.N,1.5,10)
 lhs.sample$pi.S<-qunif(lhs.sample$pi.S,0.1,0.9)
 lhs.sample$alfa.S<-qunif(lhs.sample$alfa.S,0.3,0.35)
 lhs.sample$alfa_1.S<-qunif(lhs.sample$alfa_1.S,0.1,0.9)
 lhs.sample$epsilon.S<-qunif(lhs.sample$epsilon.S,1.5,10)
 lhs.sample$Gamma_re<-qunif(lhs.sample$Gamma_re,0.0,1.5)
 lhs.sample$Gamma_ce<-qunif(lhs.sample$Gamma_ce,0.0,1.5)
 lhs.sample$Eta_re<-qunif(lhs.sample$Eta_re,0.005,0.08) #maybe higher than 0.1 might be needed
 lhs.sample$Eta_ce<-qunif(lhs.sample$Eta_ce,0.005,0.08)
 lhs.sample$Nu_re<-qunif(lhs.sample$Nu_re,0.005,0.08)
 lhs.sample$Nu_ce<-qunif(lhs.sample$Nu_ce,0.005,0.08)
...
ediamMSE(c(  round(lhs.sample[i,'alfa.N'] ,2),
             ...
             round(lhs.sample[i,'Eta_re'],2),
             round(lhs.sample[i,'Eta_ce'],2),
             round( lhs.sample[i,'Nu_re'],2),
             round(lhs.sample[i,'Nu_ce'],2)
           ), verbose = TRUE
           )
...
#multi-core runs
 ...
 global.elements<-list("ediamMSE","ediamCalib","ode","HistData","HistGDPData","HistPrice","HistPopData","ValVars","dir.model")
 clusterExport(cl,global.elements,envir=environment())
 test.out<- parApply(cl,lhs.sample,1,function(x) {out<-ediamMSE(c(  round(as.numeric(x['pi.N']),2),
                                                                    round(as.numeric(x['alfa.N']),2),
                                                                    round(as.numeric(x['alfa_1.N']),2),
                                                                    round(as.numeric(x['epsilon.N']) ,2),
                                                                    round(as.numeric(x['pi.S']),2),
                                                                    round(as.numeric(x['alfa.S']) ,2),
                                                                    round(as.numeric(x['alfa_1.S']) ,2),
                                                                    round(as.numeric(x['epsilon.S']),2),
                                                                    round(as.numeric(x['Gamma_re']) ,2),
                                                                    round(as.numeric(x['Gamma_ce']),2),
                                                                    round(as.numeric(x['Eta_re']),2),
                                                                    round(as.numeric(x['Eta_ce']),2),
                                                                    round(as.numeric(x['Nu_re']),2),
                                                                    round(as.numeric(x['Nu_ce']),2)
                                                                  ),verbose=TRUE);
...
dim(test.out)
#order
 test.out<-test.out[order(test.out$MSE.Y.N ),]
 round(test.out,4)
...
#############################################################################################
#let's look at individual policies
...
#Optimization
 library(snow)
  nCore<-40
  cl <- makeSOCKcluster(names = rep('localhost',nCore))
  global.elements<-list("ediamMSE","ediamCalib","ode","HistData","HistPrice","HistPopData","ValVars","dir.model")
  clusterExport(cl,global.elements,envir=environment())

library(rgenoud)
genoud(ediamMSE,max=FALSE,nvars=12,
       starting.values = c( 0.33, #alfa.N
                            0.5,  #alfa_1.N
                            5.0,  #epsilon.N
                            0.33, #alfa.S
                            0.50, #alfa_1.S
...
Hist<-data.frame( Time = rep(c(1980:2012),6),
            ...
            Data.type="Hist")
#
#0.01159775 0.89663578 5.52116609 0.04577620 0.77277290 9.99991257 1.13219801 0.88160998 0.01299530 0.01199288 0.01833847 0.01223270
#0.01159775 0.89663578 5.52116609 0.04577620 0.77277290 9.99991257 1.13219801 0.88160998 0.01299530 0.01199288 0.01833847 0.01223270
#0.071514700  0.740314307  3.190060395  0.010838608  0.897073498  9.979736707  0.803783585 -0.009148637 -0.006473807  0.022976643  0.013920194  0.01300690
#0.1102057 0.7953044  3.905237 0.2412745 0.8883247  8.439782 1.3528351 0.013839505 0.32210834 0.2596543 0.026054756 0.008385389
#0.1863194 0.8518879  3.042283 0.2350036 0.8964040  9.564328 1.3305200 0.003985652 0.01081649 0.02626312 0.05443534 0.014570982
#0.2150459 0.4079114  3.834902 0.1755453 0.6067945  8.895523 0.2605841 0.013047310 0.038611751 0.014856207 0.077003210 0.047584963

SimData<-ediamCalib(c( alfa.N = round(0.2150459  ,2),
                         alfa_1.N= round( 0.4079114 ,2),
                         epsilon.N = round( 3.834902 ,2),
                         alfa.S = round(0.1755453 ,2),
                         alfa_1.S = round(0.6067945 ,2),
                         epsilon.S = round(8.895523,2),
                         Gamma_re = round(0.2605841 ,2),
                         Gamma_ce = round(0.013047310  ,2),
                         Eta_re = round(0.038611751,2),
                         Eta_ce = round(0.014856207,2),
                         Nu_re = round(0.077003210 ,2),
                         Nu_ce = round(0.047584963,2)),
