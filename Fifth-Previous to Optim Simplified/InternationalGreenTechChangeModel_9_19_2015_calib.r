...
#Determine policy parameters
 policy.vector<-c(
    ...
    #Technology push in North
	 epsi.re.subsidy.N = as.numeric(policies[5]),
     s.re.subsidy.N = as.numeric(policies[6]),
    #Traditional Green Climate Fund
	 epsi.re.GFsubsidy.N = 0,
     epsi.re.subsidy.S = as.numeric(policies[7]),
    # R&D Green Climate Fund
	 s.re.subsidy.S = as.numeric(policies[8]),
     s.re.GFsubsidy.N = 0)
