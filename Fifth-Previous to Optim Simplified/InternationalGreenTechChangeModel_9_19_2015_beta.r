...
#P4.Nordhaus+R&DGreenClimateFund
 ...
 policy.vector<-c(
	...
	policy.duration=as.numeric(policies[2]), #1 
    #carbon tax
	 tax.rate.N=as.numeric(policies[3]),
     tax.rate.S=as.numeric(policies[4]),
    #Technology push in North
	 epsi.re.subsidy.N = ifelse(as.character(params['policy.name'])%in%c("Nordhauds+TechnologyPolicy","Nordhaus+TraditionalGreenClimateFund","Nordhaus+R&DGreenClimateFund",
	                                                                     "Nordhaus+TechnologyPolicy.Both","Nordhaus+TraditionalGreenClimateFund+R&DS","Nordhaus+CoR&DGreenClimateFund",
																		 "Nordhaus+CoR&DGreenClimateFund+TecS")==TRUE,as.numeric(policies[5]),0),
     
	 s.re.subsidy.N = ifelse(as.character(params['policy.name'])%in%c("Nordhauds+TechnologyPolicy","Nordhaus+TraditionalGreenClimateFund","Nordhaus+R&DGreenClimateFund",
	                                                                  "Nordhaus+TechnologyPolicy.Both","Nordhaus+TraditionalGreenClimateFund+R&DS","Nordhaus+CoR&DGreenClimateFund",
																	  "Nordhaus+CoR&DGreenClimateFund+TecS")==TRUE,as.numeric(policies[6]),0),
    #Traditional Green Climate Fund
	 epsi.re.GFsubsidy.N = ifelse(as.character(params['policy.name'])%in%c("Nordhaus+TraditionalGreenClimateFund","Nordhaus+R&DGreenClimateFund","Nordhaus+TraditionalGreenClimateFund+R&DS")==TRUE,as.numeric(policies[7]),0),
     
	 epsi.re.subsidy.S = ifelse(as.character(params['policy.name'])%in%c("Nordhaus+TechnologyPolicy.Both","Nordhaus+CoR&DGreenClimateFund+TecS")==TRUE,as.numeric(policies[7]),
	                     ifelse(as.character(params['policy.name'])%in%c("Nordhaus+TraditionalGreenClimateFund","Nordhaus+R&DGreenClimateFund","Nordhaus+TraditionalGreenClimateFund+R&DS")==TRUE,as.numeric(policies[8]),0)),
    # R&D Green Climate Fund
	 s.re.subsidy.S = ifelse(as.character(params['policy.name'])%in%c("Nordhaus+CoR&DGreenClimateFund")==TRUE,as.numeric(policies[7]),
	                  ifelse(as.character(params['policy.name'])%in%c("Nordhaus+TechnologyPolicy.Both","Nordhaus+CoR&DGreenClimateFund+TecS")==TRUE,as.numeric(policies[8]),
	                  ifelse(as.character(params['policy.name'])%in%c("Nordhaus+TraditionalGreenClimateFund+R&DS","Nordhaus+R&DGreenClimateFund")==TRUE,as.numeric(policies[9]),0))),
     
	 s.re.GFsubsidy.N = ifelse(as.character(params['policy.name'])%in%c("Nordhaus+CoR&DGreenClimateFund")==TRUE,as.numeric(policies[8]),
	                    ifelse(as.character(params['policy.name'])%in%c("Nordhaus+CoR&DGreenClimateFund+TecS")==TRUE,as.numeric(policies[9]),
                        ifelse(as.character(params['policy.name'])%in%c("Nordhaus+R&DGreenClimateFund")==TRUE,as.numeric(policies[10]),0))))
...
ModelEngine <- function(Time, State, Parameters) {
  ...
  #Policy Instruments
     #status of policy objectives
	  ...
	  #policy.status<-ifelse(dS.lag>0.0,0,1)
  ...
  #Economic structure
   ...
   #Welfare Calculations
	...
	#Utility Functions
	 #Utility.Consumer.N<-ifelse(Cost.S.Damage==0,-100,
	 #                             ifelse(Budget.function.N<0,-10000000,
  	 #                                       (1+((Cost.S.Damage*Consumption.N)^(1-sigma.utility))/(1-sigma.utility))*(1/((1+rho*5)^Time))))
	 #Utility.Consumer.S<-ifelse(Cost.S.Damage==0,-100,
	 #                             ifelse(Budget.function.S<0,-10000000,
	 #							            (1+((Cost.S.Damage*Consumption.S)^(1-sigma.utility))/(1-sigma.utility))*(1/((1+rho*5)^Time))))
   ...
   #State variables
    ...
    #Environmental Quality
	 ...
	 #dS<-min(0.1,Delta.S*S-qsi*(Yce.N+Yce.S))
