...
## =======================================================================================================================================================================================================
ediam<-function (policies,params){
...
#Load parameters required for determining initial conditions
  ...
  CO2.Concentration.0<-as.numeric(params['CO2.Concentration.0'])
...
Parameters <- c(alfa = as.numeric(params['alfa']),
                ...
                policy.half.life = as.numeric(params['policy.half.life']),
                policy.vector)
#==========================================================================================================================================================================================================
...
EdiamModelEngine <- function(Time, State, Parameters) {
  ...
  #Policy Instruments
	 #policies in the Advanced Region
	    ce.tax.N<-ifelse(Time<=10,0,tax.rate.N*exp(-1*0.01*Time))
      RD.subsidy.N<-ifelse(Time<=10,0,s.re.subsidy.N*exp(-1*0.01*Time))
	    RD.subsidy.GF.N<-ifelse(Time<=10,0,s.re.GFsubsidy.N*exp(-1*0.01*Time))
	    Tec.subsidy.N<-ifelse(Time<=10,0,epsi.re.subsidy.N*exp(-1*0.01*Time))
	    Tec.subsidy.GF.N<-ifelse(Time<=10,0,epsi.re.GFsubsidy.N*exp(-1*0.01*Time))

	 #policies in the Emerging Region
	    ce.tax.S<-ifelse(Time<=10,0,tax.rate.S*exp(-1*0.01*Time))
      RD.subsidy.S<-ifelse(Time<=10,0,s.re.subsidy.S*exp(-1*0.01*Time))
	    Tec.subsidy.S<-ifelse(Time<=10,0,epsi.re.subsidy.S*exp(-1*0.01*Time))
