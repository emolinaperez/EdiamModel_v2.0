...
#check that merges checks
  ...
  HistData<-Reduce(function(...) { merge(..., all=TRUE) }, list(HistData,oil.data,HistPrice))
...
#subset the data to the years that we have data for
   ...
   HistData$Y_ce.h<-HistData[,"TPES"]*(1-HistData[,"RENTPES"]) #MTOE
...
#Create NonOECD region
  ...
  HistDataNonOECD$COAL<-HistDataNonOECD$COAL-HistData$COAL[HistData$Country=="Memo: OECD Total"]
  HistDataNonOECD$MTOTOIL<-HistDataNonOECD$MTOTOIL-HistData$MTOTOIL[HistData$Country=="Memo: OECD Total"]
  HistDataNonOECD$NATGAS<-HistDataNonOECD$NATGAS-HistData$NATGAS[HistData$Country=="Memo: OECD Total"]
...
#Additional operations
   HistData$Oil.Factor<-HistData[,"MTOTOIL"]/(HistData[,"COAL"]+HistData[,"MTOTOIL"]+HistData[,"NATGAS"]) #[1]
   HistData$Roil.h<-HistData[,"OILTPES"] #MTOE
   HistData$Re.h<-HistData$Roil.h/HistData$Oil.Factor #MTOE
   ...
   HistData$Re.Expenditure.h<-HistData$Price.Oil*HistData$Oil.Factor*HistData$Roil.h # [Billion USD]
   HistData$ReToGDP.h<-HistData$Re.Expenditure.h/HistData$GDP # [1]
...
#Oil prices
  #cR.y<<-(HistPrice$cR*7.1428571428571)*(1e6)/(1e9) #Billion2010USD/Mtoe
...
#Optimization
 library(snow)
  nCore<-40
  cl <- makeSOCKcluster(names = rep('localhost',nCore))
  global.elements<-list("ediamMSE","ediamCalib","HistData","HistPrice","ValVars","dir.model")
  clusterExport(cl,global.elements,envir=environment())
