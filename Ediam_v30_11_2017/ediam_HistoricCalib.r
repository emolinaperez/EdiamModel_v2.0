...
#==========================================================================================================================================================================================================
 Parameters <- c(Run.ID = 1, #as.numeric(params['Run.ID'])
                 EndTime = (length(Price.oil.y)-1), #as.numeric(params['EndTime'])
                 TimeStep = 1, #as.numeric(params['TimeStep'])
                #economic parameters North
                 alfa.N = as.numeric(calib.params['alfa.N']),# as.numeric(0.48), #
                ...
                #Technological paramters
                 ...
                 val.param = as.numeric(calib.params['val.param']),
                 val.param.S = as.numeric(calib.params['val.param.S']),
                 pi.N = as.numeric(calib.params['pi.N']), #1.0, #maybe 2.5 is not a bad value
                 pi.S = as.numeric(calib.params['pi.S']), #1.0
                 lrng.re = as.numeric(calib.params['lrng.re']), # .005
                 lrng.ce = as.numeric(calib.params['lrng.ce']), # .005
                 lrng.re.S = as.numeric(calib.params['lrng.re.S']), # .005
                 lrng.ce.S = as.numeric(calib.params['lrng.ce.S']), # .005
                 cf.re.N = as.numeric(calib.params['cf.re.N']),
                 cf.ce.N = as.numeric(calib.params['cf.ce.N']),
                 cf.re.S = as.numeric(calib.params['cf.re.S']),
                 cf.ce.S = as.numeric(calib.params['cf.ce.S'])
                )
...
# Estimate empirical parameters
 EmpiricalParams<-apply(
                         #data.frame(index= seq(1,Parameters['EndTime'],by=Parameters['TimeStep'])),
                         data.frame(index= seq(1,5,by=1)),
...
#for Oil
 Price.oil.Data <<- approxfun( x = seq(0,Parameters['EndTime'],1), #times
                       ...
                       rule = 2)
#for final good
 Price_Y.N.Data <<- approxfun( x = seq(0,Parameters['EndTime'],1), #times
                      y = Price.oil.y, # Actual data points
                      method = "linear",
                      rule = 2)
