...
#Determine policy parameters
 policy.vector<-c(
	...
	policy.start.time = 0.034,
	#Duration of policies
	policy.duration=as.numeric(policies[1]),
    #carbon tax
	 tax.rate.N=as.numeric(policies[2]),
     tax.rate.S=as.numeric(policies[3]),
    #Technology push in North
	 epsi.re.subsidy.N = ifelse(as.character(params['policy.name'])%in%c("Nordhauds+TechnologyPolicy","Nordhaus+TraditionalGreenClimateFund","Nordhaus+R&DGreenClimateFund")==TRUE,as.numeric(policies[4]),0),
     s.re.subsidy.N = ifelse(as.character(params['policy.name'])%in%c("Nordhauds+TechnologyPolicy","Nordhaus+TraditionalGreenClimateFund","Nordhaus+R&DGreenClimateFund")==TRUE,as.numeric(policies[5]),0),
    #Traditional Green Climate Fund
	 epsi.re.GFsubsidy.N = ifelse(as.character(params['policy.name'])%in%c("Nordhaus+TraditionalGreenClimateFund","Nordhaus+R&DGreenClimateFund")==TRUE,as.numeric(policies[6]),0),
     epsi.re.subsidy.S = ifelse(as.character(params['policy.name'])%in%c("Nordhaus+TraditionalGreenClimateFund","Nordhaus+R&DGreenClimateFund")==TRUE,as.numeric(policies[7]),0),
    # R&D Green Climate Fund
	 s.re.subsidy.S = ifelse(as.character(params['policy.name'])%in%c("Nordhaus+R&DGreenClimateFund")==TRUE,as.numeric(policies[8]),0),
     s.re.GFsubsidy.N = ifelse(as.character(params['policy.name'])%in%c("Nordhaus+R&DGreenClimateFund")==TRUE,as.numeric(policies[9]),0))
  ...
  #Economic structure
    ...
    #Second we determine the equilibrium conditions for each sector
     #clean sector
       ...
       Profits.re.N<-(1+RD.subsidy.N)*((epsi.re/alfa)-epsi.re)*Agg.demand.re.tech.N
     ...
     #dirty sector
	   ...
	   Profits.ce.N<-((epsi.ce/alfa)-epsi.ce)*Agg.demand.ce.tech.N
    ...
    #Second we determine the equilibrium conditions for each sector
     #clean sector
	   ...
	   Profits.re.S<-(1+RD.subsidy.S)*((epsi.re/alfa)-epsi.re)*Agg.demand.re.tech.S
     ...
     #dirty sector
	   ...
	   Profits.ce.S<-((epsi.ce/alfa)-epsi.ce)*Agg.demand.ce.tech.S
   ...
   #Welfare Calculations
     Consumption.N<-Y.N-
					...
					#RD.subsidy.GF.N*((epsi.re/alfa)-epsi.re)*Agg.demand.re.tech.S #cost of green climate fund R&D subsidies 
     Consumption.S<-(Y.S -
     ...
     Utility.Consumer.N<-ifelse(Cost.S.Damage==0,-10,(1+((Cost.S.Damage*Consumption.N)^(1-sigma.utility))/(1-sigma.utility))*(1/((1+rho*5)^Time)))
	 Utility.Consumer.S<-ifelse(Cost.S.Damage==0,-10,(1+((Cost.S.Damage*Consumption.S)^(1-sigma.utility))/(1-sigma.utility))*(1/((1+rho*5)^Time)))
