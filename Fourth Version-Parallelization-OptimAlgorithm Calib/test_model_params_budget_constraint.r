...
#Source Experimental Design
  ...
  #experiment.version<-"Exp.design_p0.csv"
       ...
       if (x['policy.name']=="Nordhaus")
        {
		 ...
		 {
		  optimx(c(0.033,0.2,0.25,0.25,0.10,2.0), TechChangeMod, lower=c(0.03,0.1,0.05,0.05,0.01,0.5), upper=c(0.5,1.0,0.5,0.5,0.15,2.0),method="L-BFGS-B",control = list(fnscale = -1,ndeps=c(0.01,0.01,0.01,0.01,0.01,0.01),parscale=c(1,10,10,10,2,2),maxit=200000),params=params)
		 } else {
		   if (x['policy.name']=="Nordhaus+TraditionalGreenClimateFund")
		  {
		   	optimx(c(0.033,0.2,0.25,0.25,0.10,2.0,0.05,0.05), TechChangeMod, lower=c(0.03,0.1,0.05,0.05,0.01,0.5,0.01,0.01), upper=c(0.5,1.0,0.5,0.5,0.15,2.0,0.10,0.10),method="L-BFGS-B",control = list(fnscale = -1,ndeps=c(0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01),parscale=c(1,10,10,10,2,2,2,2),maxit=200000),params=params)
		  } else {
		    if (x['policy.name']=="Nordhaus+R&DGreenClimateFund")
		     {
			  optimx(c(0.033,0.2,0.25,0.25,0.10,2.0,0.05,0.05,0.5,0.5), TechChangeMod, lower=c(0.03,0.1,0.05,0.05,0.01,0.5,0.01,0.01,0.01,0.01), upper=c(0.5,1.0,0.5,0.5,0.15,2.0,0.10,0.10,1.0,1.0),method="L-BFGS-B",control = list(fnscale = -1,ndeps=c(0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01),parscale=c(1,10,10,10,2,2,2,2,2,2),maxit=200000),params=params)
			 } else {
