       ...
       if (x['policy.name']=="Nordhaus")
        {
		 ...
		 {
		  optimx(c(0.033,0.2,0.25,0.25,0.05,1.0), TechChangeMod, lower=c(0.03,0.1,0.01,0.01,0.01,0.5), upper=c(0.5,1.0,0.5,0.5,0.1,2),method="L-BFGS-B",control = list(fnscale = -1,ndeps=c(0.01,0.1,0.01,0.01,0.01,0.01),maxit=200000),params=params)
		 } else {
		   if (x['policy.name']=="Nordhaus+TraditionalGreenClimateFund")
		  {
		   	optimx(c(0.033,0.2,0.25,0.25,0.05,1.0,0.05,0.05), TechChangeMod, lower=c(0.03,0.1,0.01,0.01,0.01,0.5,0.01,0.01), upper=c(0.5,1.0,0.5,0.5,0.1,2,0.05,0.05),method="L-BFGS-B",control = list(fnscale = -1,ndeps=c(0.01,0.1,0.01,0.01,0.01,0.01,0.01,0.01),maxit=200000),params=params)
		  } else {
		    if (x['policy.name']=="Nordhaus+R&DGreenClimateFund")
		     {
			  optimx(c(0.033,0.2,0.25,0.25,0.05,1.0,0.05,0.05,0.5,0.5), TechChangeMod, lower=c(0.03,0.1,0.01,0.01,0.01,0.5,0.01,0.01,0.01,0.01), upper=c(0.5,1.0,0.5,0.5,0.1,2,0.05,0.05,0.5,0.5),method="L-BFGS-B",control = list(fnscale = -1,ndeps=c(0.01,0.1,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01),maxit=200000),params=params)
			 } else {
