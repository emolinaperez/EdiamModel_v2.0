...
ModelEngine <- function(Time, State, Parameters) {
  ...
  #Economic structure
   ...
   #Welfare Calculations
     ...
     Utility.Consumer.N<-ifelse(Cost.S.Damage==0,-1,(1+((Cost.S.Damage*Consumption.N)^(1-sigma.utility))/(1-sigma.utility))*(1/((1+rho*5)^Time)))
	 Utility.Consumer.S<-ifelse(Cost.S.Damage==0,-1,(1+((Cost.S.Damage*Consumption.S)^(1-sigma.utility))/(1-sigma.utility))*(1/((1+rho*5)^Time)))
...
#Model Solver
 ...
 #collapse<-ifelse(0%in%out$Cost.S.Damage,TRUE,FALSE)
 ...
 #consum$Utility.Consumer.Total_N<-ifelse(collapse==TRUE,-500,consum$Utility.Consumer.Total_N)
 #consum$Utility.Consumer.Total_S<-ifelse(collapse==TRUE,-500,consum$Utility.Consumer.Total_S) 
